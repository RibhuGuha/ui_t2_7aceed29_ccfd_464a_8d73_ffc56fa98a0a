...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, propertyId;
        ...
        rslt = await Support.AddOrUpdateProperty(property, dropDownOptions, []);
        if (rslt.status) {
            propertyId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Property and navigation entity if it is deos not exist
            let navItem = property.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "PropId", value: parseInt(propertyId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateProperty(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Property is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
